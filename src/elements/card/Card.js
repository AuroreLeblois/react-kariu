...
const Card = (props) => {
  const [showContent, setShowContent] = useState(props.showContent || true);
  ...
  const styleCursor = {
    cursor:
      props.onClick || props.variant === "accordion" ? "pointer" : "default",
  };
  ...
  const styleCard = {
    ...
    ":hover": {
      opacity: props.onClick || props.variant === "accordion" ? 0.85 : 1,
    },
  };
  if (props.loading) {
    return (
    ...
    return (
      <div className={`card-kariu ${css(styleCard)} `} onClick={props.onClick}>
        {renderTitle(
          styleCursor,
          textColor,
          props.title,
        ...
        {renderContent(showContent, props)}
      </div>
    );
  }
};
...
function renderTitle(
  ...
  const styleTitleContainer = {
    ...
    userSelect: "none",
    button: {
      marginLeft: variant === "accordion" ? "1rem" : "0",
    },
  };
  let rotate = {
    svg: {
...
function renderContent(showContent, props) {
