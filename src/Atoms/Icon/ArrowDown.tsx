...
function Arrow({size = 24, color = null, direction = "down", style = {}, onClick = () => {}}){
  const { colors } = useTheme();
  if (!size) throw new Error;

  return (
    <div className="arrow-container" onClick={onClick} style={style}>
      <svg width={`${size}px`} height={`${size}px`} 
      viewBox="0 0 24 24" fill={color ?? colors.text} xmlns="http://www.w3.org/2000/svg">
...
9.31655 17.2937 9.31655 16.9032 9.70708L12.7176 13.8927C12.3271 14.2833 11.6939 14.2832 11.3034 13.8927L7.12132 9.71069C6.7308 9.32016
 6.09763
 9.32016 5.70711 9.71069Z" fill={color ?? colors.text}/>
      </svg>
    </div>
  );
}
